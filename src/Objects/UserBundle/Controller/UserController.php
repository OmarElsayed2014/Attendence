...
class UserController extends Controller {
    ...
    public function homeAction(){
        ...
        $attendenceRepository = $doctrine->getRepository("ObjectsAttendenceBundle:Attendence");
        $userRepository = $doctrine->getRepository("ObjectsUserBundle:User");

        $isCheckin = true;
        $isCheckout = true;

        $checkedinObject = $attendenceRepository->isCheckedin($userId, $todayDate);
        $checkedoutObject = $attendenceRepository->isCheckedout($userId, $todayDate);
        ...
        $form = $this->createFormBuilder()
                 ->add('checkin','submit', array('disabled' => $isCheckin))
                 ->add('check_location','text',
                         array('attr' => array('readonly' => true,'value' => "Loading...")))
                 ->add('checkout','submit',array('disabled' => $isCheckout))
                 ->getForm();
        ...
        if($request->isMethod("POST")) {
            ...
            if($form->isValid()) {
                ...
                $location = $form->get("check_location")->getData();

                if($location != "Loading...") {

                    if($form->get("checkin")->isClicked()) {

                        $vacationDay = false;

                        $attendenceObject = new \Objects\AttendenceBundle\Entity\Attendence();

                        $attendenceObject->setCheckin($now);
                        $attendenceObject->setLocationCheckin($location);
                        $attendenceObject->setUser($userRepository->find($userId));

                        $dayIndex = $now->format("N");

                        // Get configuartion variables
                        $weekVacations = $this->container->getParameter('week_vecations');
                        $weekVacationsArray  = array_map('intval', str_split($weekVacations));

                        foreach ( $weekVacationsArray as $weekendDay ) {
                            if($dayIndex == $weekendDay) {
                                $vacationDay = true;
                            }
                        }

                        if(!$vacationDay) {
                            $maxCheckinTime = $this->container->getParameter('max_checkin');

                            $checkInCalculated = new \DateTime($now->format("H:i"));
                            ...
                            $attendenceObject->setMinus($minus);
                        }
                        $doctrineManager->persist($attendenceObject);

                        $request->getSession()->getFlashBag()->add('success', 'You has been Checked In  Successfully');
                    }
                    else {
                        if(isset($checkedinObject)) {
                            $checkout = $checkedinObject[0];
                            $minus = $checkout->getMinus();

                            $overtimeDay = false;

                            $checkout->setCheckout($now);
                            ...
                            $weekVacationsOvertime = $this->container->getParameter('week_vecations_overtime');
                            $weekVacationsOvertimeArray  = array_map('intval', str_split($weekVacationsOvertime));

                            foreach ( $weekVacationsOvertimeArray as $weekendDay ) {
                                if($dayIndex == $weekendDay) {
                                    $overtimeDay = true;
                                }
                            }
                            ...
                            $plus = ($overtimeDay)?$noOfHrs:0;

                            $checkout->setPlus($plus);
                            $checkout->setNoOfHrs($noOfHrs);
                            $checkout->setTotal($noOfHrs + $plus - $minus);
                            $checkout->setLocationCheckout($location);

                            $request->getSession()->getFlashBag()->add('success', 'You has been Checked Out  Successfully');
                        }
                    }
                }
                else {
                    $request->getSession()->getFlashBag()->add('error', 'Sorry, Cannot checkin before Geo location is returned');
        ...
        return $this->render("ObjectsUserBundle::home.html.twig", array('form' => $form->createView()));
    }
